...
static bool parse_token_balance_object_with_owner(
    ...
    // (Parsing logic identical to your provided code)
...
// --- HELPER: Flush Builders to Flight ---
arrow::Status FlushBuildersToFlight(
    ...
    // Finish builders (creates arrays and resets builders)
...
// Now returns void because data is sent, not collected.
void process_transactions_parallel(
    ...
    auto worker_fn = [&](size_t thread_id) {
        ...
        // --- FIX 1: Updated Location::Parse API ---
        ...
        // --- FIX 2: Updated FlightClient::Connect API ---
        ...
        // --- FIX 3: Updated DoPut API ---
        auto descriptor = arrow::flight::FlightDescriptor::Path({"solana_data"});
        auto schema = GetOutputSchema();
        
        // DoPut now returns a Result containing a struct with the writer and reader
        ...
        // Extract writer and reader from the result struct
        ...
        // 4. Processing Loop (Logic Unchanged)
        ...
        for (size_t tx_idx = start_tx; tx_idx < end_tx; ++tx_idx) {
            ...
            // (Reuse your existing logic for filtering and parsing...)
            bool is_pool_tx =
            ...
            if (builders.tx_wallet_builder.length() >= BATCH_SIZE_THRESHOLD) {
                arrow::Status status = FlushBuildersToFlight(builders, writer.get());
                if (!status.ok()) {
