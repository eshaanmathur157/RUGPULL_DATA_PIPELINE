...
HotAddressLookups load_hot_addresses(const char* filepath) {
    ...
    try {
        ...
        std::cout << "Loaded and hashed " << lookups.storage.size() 
                  << " hot addresses from " << filepath << std::endl;

        if (lookups.storage.empty()) {
             std::cerr << "Warning: No addresses loaded from file: " << filepath << std::endl;
        }

    } catch (const std::exception& e) {
        std::cerr << "Base/Quotes File I/O Error: " << e.what() << std::endl;
        exit(1);
    }
    return lookups;
}
